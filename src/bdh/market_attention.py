...
# --- 3. Base Class for Market BDH Models ---
class MarketBDHBase(nn.Module):
    """
    Base class for Market BDH models with attention-based architecture.
    Contains shared core architecture: input adapter + BDH layers.
    """
    def __init__(
        self,
        num_assets: int,
        ...
        nn.init.normal_(self.input_adapter.weight, mean=0.0, std=0.02)
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            state: Hidden representation [B, T, D]
        """
        B, T, N, F = x.shape
